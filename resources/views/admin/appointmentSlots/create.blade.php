...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.appointmentSlot.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.appointment-slots.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('select_doctor') ? 'is-invalid' : '' }}" name="select_doctor_id" id="select_doctor_id" required>
                    @foreach($select_doctors as $id => $entry)
                        <option value="{{ $id }}" {{ old('select_doctor_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('select_time') ? 'is-invalid' : '' }}" type="text" name="select_time" id="select_time" value="{{ old('select_time') }}" required>
