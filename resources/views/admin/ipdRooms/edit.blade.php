...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.ipdRoom.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.ipd-rooms.update", [$ipdRoom->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="room_no">{{ trans('cruds.ipdRoom.fields.room_no') }}</label>
                <input class="form-control {{ $errors->has('room_no') ? 'is-invalid' : '' }}" type="text" name="room_no" id="room_no" value="{{ old('room_no', $ipdRoom->room_no) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('ward_type') ? 'is-invalid' : '' }}" name="ward_type" id="ward_type">
                    ...
                    @foreach(App\Models\IpdRoom::WARD_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('ward_type', $ipdRoom->ward_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('charges_per_day') ? 'is-invalid' : '' }}" type="number" name="charges_per_day" id="charges_per_day" value="{{ old('charges_per_day', $ipdRoom->charges_per_day) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('notes') ? 'is-invalid' : '' }}" name="notes" id="notes">{!! old('notes', $ipdRoom->notes) !!}</textarea>
                @if($errors->has('notes'))
                    <div class="invalid-feedback">
                        {{ $errors->first('notes') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\IpdRoom::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $ipdRoom->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
