...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.ipdMedication.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.ipd-medications.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('frequency') ? 'is-invalid' : '' }}" type="text" name="frequency" id="frequency" value="{{ old('frequency', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('route') ? 'is-invalid' : '' }}" name="route" id="route">
                    ...
                    @foreach(App\Models\IpdMedication::ROUTE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('route', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('route'))
                    <div class="invalid-feedback">
