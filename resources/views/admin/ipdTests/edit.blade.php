...
<div class="card custom-card">
    <div class="card-header bg-primary text-white card-header-compact">
        <strong>{{ trans('global.edit') }} {{ trans('cruds.ipdTest.title_singular') }}</strong>
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route('admin.ipd-tests.update', [$ipdTest->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2 {{ $errors->has('ipd') ? 'is-invalid' : '' }}" name="ipd_id" id="ipd_id">
                            @foreach($ipds as $id => $entry)
                                <option value="{{ $id }}" {{ (old('ipd_id') ? old('ipd_id') == $id : $ipdTest->ipd_id == $id) ? 'selected' : '' }}>{{ $entry }}</option>
                            @endforeach
                        </select>
                        @if($errors->has('ipd'))
            ...
            <div class="row mt-3">
                ...
                <div class="form-group col-lg-6">
                    ...
                    <select class="form-control select2 {{ $errors->has('tests') ? 'is-invalid' : '' }}" name="tests[]" id="tests" multiple>
                        @foreach($tests as $id => $test)
                            <option value="{{ $id }}" {{ (in_array($id, old('tests', [])) || $ipdTest->tests->contains($id)) ? 'selected' : '' }}>{{ $test }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('tests'))
                ...
                <div class="form-group col-lg-6">
                    ...
                    <textarea class="form-control ckeditor {{ $errors->has('notes') ? 'is-invalid' : '' }}" name="notes" id="notes">{!! old('notes', $ipdTest->notes) !!}</textarea>
                    @if($errors->has('notes'))
...
$(document).ready(function () {
  let initialIpd = '{{ old('ipd_id', $ipdTest->ipd_id) }}';
  if (initialIpd) {
    $('#ipd-cards').show();
