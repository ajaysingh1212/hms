...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.addDoctor.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.add-doctors.update", [$addDoctor->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('select_department') ? 'is-invalid' : '' }}" name="select_department_id" id="select_department_id">
                    @foreach($select_departments as $id => $entry)
                        <option value="{{ $id }}" {{ (old('select_department_id') ? old('select_department_id') : $addDoctor->select_department->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('doctor_name') ? 'is-invalid' : '' }}" type="text" name="doctor_name" id="doctor_name" value="{{ old('doctor_name', $addDoctor->doctor_name) }}" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\AddDoctor::AVAILABLE_DAYS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('available_days') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="available_days_{{ $key }}" name="available_days" value="{{ $key }}" {{ old('available_days', $addDoctor->available_days) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('appointment_slot_duration') ? 'is-invalid' : '' }}" type="text" name="appointment_slot_duration" id="appointment_slot_duration" value="{{ old('appointment_slot_duration', $addDoctor->appointment_slot_duration) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('max_patients_per_day') ? 'is-invalid' : '' }}" type="text" name="max_patients_per_day" id="max_patients_per_day" value="{{ old('max_patients_per_day', $addDoctor->max_patients_per_day) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('doctor_fee') ? 'is-invalid' : '' }}" type="number" name="doctor_fee" id="doctor_fee" value="{{ old('doctor_fee', $addDoctor->doctor_fee) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $addDoctor->description) !!}</textarea>
