    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.appointments.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('patient_number') ? 'is-invalid' : '' }}" type="text" name="patient_number" id="patient_number" value="{{ old('patient_number', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('available_slots') ? 'is-invalid' : '' }}" name="available_slots_id" id="available_slots_id">
                    @foreach($available_slots as $id => $entry)
                        <option value="{{ $id }}" {{ old('available_slots_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('reason_for_visit') ? 'is-invalid' : '' }}" name="reason_for_visit" id="reason_for_visit">{!! old('reason_for_visit') !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('appointment_type') ? 'is-invalid' : '' }}" name="appointment_type" id="appointment_type">
                    ...
                    @foreach(App\Models\Appointment::APPOINTMENT_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('appointment_type', 'New') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
